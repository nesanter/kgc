...
DRUNTIMENAME=libdruntime-kgc
PHOBOSNAME=libphobos2-kgc
...
ifeq (,$(findstring win,$(OS)))
	DRUNTIME = $(DRUNTIME_PATH)/lib/$(DRUNTIMENAME)-$(OS)$(MODEL).a
	DRUNTIMESO = $(DRUNTIME_PATH)/lib/$(DRUNTIMENAME)-$(OS)$(MODEL)so.a
else
...
SONAME = $(PHOBOSNAME).so.0.$(shell awk -F. '{ print $$2 + 0 }' $(VERSION))

# Set LIB, the ultimate target
ifeq (,$(findstring win,$(OS)))
	LIB = $(ROOT)/$(PHOBOSNAME).a
	LIBSO = $(ROOT)/$(SONAME).0
else
	LIB = $(ROOT)/phobos.lib
endif
...
dll : $(ROOT)/$(PHOBOSNAME).so

$(ROOT)/$(PHOBOSNAME).so: $(ROOT)/$(SONAME)
	ln -sf $(notdir $(LIBSO)) $@
...
$(PHOBOSNAME).a : generated/osx/release/32/$(PHOBOSNAME).a generated/osx/release/64/$(PHOBOSNAME).a
	lipo generated/osx/release/32/$(PHOBOSNAME).a generated/osx/release/64/$(PHOBOSNAME).a -create -output generated/osx/release/$(PHOBOSNAME).a
endif
...
UT_LIBSO:=$(ROOT)/unittest/$(PHOBOSNAME)-ut.so
