...
private
{
    ...
    extern (C) size_t gc_getBytesAllocated() nothrow;
    extern (C) size_t gc_getBytesReleased() nothrow;
    extern (C) bool gc_wait(bool full);
...
struct GC
{
    ...
    /**
     * Dumps statistics related to the garbage collector
     */
    static void dump()
    {
        gc_dump();
    ...
    /**
     * Determine how many bytes have been allocated
     * by the garbage collector
     */
    static size_t getBytesAllocated() nothrow
    {
        return gc_getBytesAllocated;
    }
    
    /**
     * Determine how many bytes have have been freed
     * by the garbage collector
     */
    static size_t getBytesReleased() nothrow
    {
        return gc_getBytesReleased();
    }
    
    /**
     * Register a function name with the garbage collector's
     * grapher module
     * (Only enabled when GRAPHER=ON is passed to the makefile)
     */
    version (GCOUT) {
        static void registerFunction(string name = __FUNCTION__)
        {
            gc_registerFunction(name);
        }
    }
    
    /**
     * Wait for the garbage collector to finish a collect cycle
     * 
     * Params:
     *  full = if true, wait for multiple cycles to complete
     */
    static void wait(bool full)
    {
        gc_wait(full);
    }
    /**
     * Output a snapshot of the current connectivity graph
     * (Only enabled when GRAPHER=ON is passed to the makefile)
     * 
     * Params:
     *  full = if true, include dead functions
