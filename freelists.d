...
struct Freelist {
	...
	void* grab(size_t sz, size_t* alloc_size = null) {
		...
		if (tail is null) {
			...
			if (tail is null)
				onOutOfMemoryError();
			*tail = Region(mem_alloc(bytes), _gc._gc_epoch % 3, sz, bytes, null);
			if (tail.ptr is null)
				onOutOfMemoryError();
		...
		if (r is null)
			onOutOfMemoryError();
		*r = Region(mem_alloc(bytes), _gc._gc_epoch % 3, sz, bytes, tail);
		if (r.ptr is null)
			onOutOfMemoryError();
		tail = r;
	...
	//this will only be called on the secondaryFL
	bool free(void* p, size_t* free_size = null) {
		debug (USAGE) printf("<GC> Freelist.free (%p)\n",p);
		Region* r = tail, rprev = null;
		while (r !is null) {
			if (r.ptr == p) {
				if (rprev !is null) {
